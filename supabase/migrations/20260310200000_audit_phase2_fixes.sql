-- M7: Partial Unique Index on Subscriptions
-- Prevents multiple active subscriptions per user.
-- The app trusts profiles.subscription_tier as the source of truth,
-- but without this constraint, billing reconciliation can have integrity issues.
CREATE UNIQUE INDEX IF NOT EXISTS idx_subscriptions_user_active
ON subscriptions(user_id)
WHERE status IN ('active', 'cancelled_pending');


-- M1: Fix relationship_state Race Condition
-- The increment_profile_counters RPC previously used COALESCE(p_relationship_state, relationship_state)
-- which does a full JSON replacement. Two rapid concurrent calls = second one overwrites the first's
-- deltas entirely. Fix: use jsonb merge (||) so concurrent calls merge their deltas.
